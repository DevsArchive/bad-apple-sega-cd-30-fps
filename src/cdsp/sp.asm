...
.WaitMain3:
	...
	btst	#2,GA_MEM_MODE+1.w		; Are we in 1M/1M mode?
	bne.s	.NoSend				; If so, branch

.SendWordRAM2:					; Give the Main CPU Word RAM access
	bset	#0,GA_MEM_MODE+1.w
	beq.s	.SendWordRAM2
...
.Commands:
	...
	bra.w	SP_ReadSectors			; Read sectors
	bra.w	SP_ReadNextSectors		; Read next sectors
	bra.w	SP_GetFileSector		; Get file sector
	bra.w	SP_LoadFile			; Load file
	bra.w	SP_SetWordRAMMode		; Set Word RAM mode
	bra.w	SP_SwapWordRAMBanks		; Swap Word RAM banks
	bra.w	SP_RunModule			; Run modules
	bra.w	SP_StopCDDA			; Stop CDDA
...
; -------------------------------------------------------------------------
; Command to read CD sectors
; -------------------------------------------------------------------------
; PARAMETERS:
;	Cmd 0	- Starting sector
;	Cmd 2	- Sector count
;	Cmd 4-6	- Destination buffer
; -------------------------------------------------------------------------

SP_ReadSectors:
	lea	CDReadVars(pc),a6		; Set up parameters
	clr.w	(a6)
	clr.w	4(a6)
	move.w	GA_CMD_0.w,0+2(a6)
	move.w	GA_CMD_2.w,4+2(a6)
	move.l	GA_CMD_4.w,8(a6)

	bra.w	ReadCD				; Read sectors

; -------------------------------------------------------------------------
; Command to read next CD sector
; -------------------------------------------------------------------------
; PARAMETERS:
;	Cmd 0	- Sector count
;	Cmd 2-4	- Destination buffer
; -------------------------------------------------------------------------
...
SP_ReadNextSectors:
	...
	move.l	GA_CMD_2.w,8(a6)
	
	bra.w	ReadCD				; Read sectors

; -------------------------------------------------------------------------
; Command to get file sector
; -------------------------------------------------------------------------
; PARAMETERS:
;	Cmd 0-A	- File name
; RETURNS:
;	Stat 0	- File sector (-1 if not found)
; -------------------------------------------------------------------------

SP_GetFileSector:
	lea	Buffer(pc),a0			; Find file
	lea	GA_CMD_0.w,a1
	move.l	(a1)+,(a0)
	move.l	(a1)+,4(a0)
	move.l	(a1),8(a0)
	...
	move.l	d0,GA_STAT_0.w			; Return file sector
	rts
...
SP_SetWordRAMMode:
	move.b	GA_MEM_MODE+1.w,d0		; Get Word RAM mode base
	andi.b	#$E2,d0

	move.w	GA_CMD_0.w,d1			; Set mode
	add.b	d1,d1
	add.b	d1,d1
	andi.b	#4,d1
	or.b	d1,d0
...
; -------------------------------------------------------------------------
; Command to swap Word RAM banks
; -------------------------------------------------------------------------

SP_SwapWordRAMBanks:
	btst	#2,GA_MEM_MODE+1.w		; Don't do this in 2M mode
	beq.s	.2MMode
...
; -------------------------------------------------------------------------
; Command to run Sub CPU module
; -------------------------------------------------------------------------

SP_RunModule:
.SendWordRAM:					; Give the Main CPU Word RAM access
	bset	#0,GA_MEM_MODE+1.w
	beq.s	.SendWordRAM

	jmp	PRG_RAM+$20000			; Run module
